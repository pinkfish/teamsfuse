...
class TeamDetails extends StatelessWidget {
  ...
  Widget _buildSeasons(
      BuildContext context, Team team, Club club, Iterable<Season> seasons) {
    List<Widget> ret = <Widget>[];

    if (team.isAdmin(club)) {
      ret.add(
        new FlatButton(
          onPressed: () =>
    ...
    if (seasons.length == 0) {
      ...
      for (Season season in seasons) {
        happyData.add(_buildSeasonExpansionTitle(team, season));
      }
    }
    ret.add(
      Column(
              ...
              builder: (BuildContext context, SingleTeamState teamState) {
                if (teamState is SingleTeamDeleted) {
                  return Column(
                    ...
                    children: <Widget>[
                      ...
                      _buildSeasons(context, teamState.team, teamState.club,
                          teamState.fullSeason)
                    ],
                  );
                }
                ...
                return new Column(
                  ...
                  children: <Widget>[
                    ...
                    _buildSeasons(
                        context, team, teamState.club, teamState.fullSeason)
                  ],
                );
              },
            ),
