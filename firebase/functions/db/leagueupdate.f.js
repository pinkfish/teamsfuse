...
exports = module.exports = functions.firestore.document('/League/{leagueId}').onUpdate((inputData, context) => {
    const finalRet = [];
...
// Handle the creation case as well, so if we create a game
// with a specific result we update the team values.
exports = module.exports = functions.firestore.document('/League/{leagueId}').onCreate((snap, context) => {
    algolia.updateLeague(snap);
});

// Handle the creation case as well, so if we create a game
// with a specific result we update the team values.
exports = module.exports = functions.firestore.document('/League/{leagueId}').onDelete((snap, context) => {
