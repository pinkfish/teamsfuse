    ...
    index.browse(
        ...
        (err, content) => {
            ...
            toUpdateRecords.forEach(id => {
                const data = { leagueDivisonName: leagueDoc.name, objectID: id };
                records.push(data);
            });
...
exports.updateLeagueSeason = teamDoc => {
    index.browse(
        ...
        (err, content) => {
            ...
            const records = [];
            toUpdateRecords.forEach(id => {
                const data = { leagueSeasonName: teamDoc.name, objectID: id };
                records.push(data);
            });
            teamIndex.partialUpdateObjects(records);
        },
...
exports.updateLeagueDivison = teamDoc => {
    index.browse(
        ...
        (err, content) => {
            ...
            const records = [];
            toUpdateRecords.forEach(id => {
                const data = { leagueDivisonName: teamDoc.name, objectID: id };
                records.push(data);
            });
            teamIndex.partialUpdateObjects(records);
        },
