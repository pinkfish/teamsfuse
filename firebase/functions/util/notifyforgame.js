...
exports.notifyForGame = (
  ...
  var sharedGame =
    sharedGameData === null ? sharedGameRef.get() : sharedGameData;
      ...
      for (const playerId in seasonData.players) {
        if (seasonData.players.hasOwnProperty(playerId)) {
          ...
          const players = db
            ...
            .get();
          promises.push(
            Promise.all([[game, team, opponent, season, sharedGame], players])
          );
      ...
      for (const key in allTokens) {
        if (allTokens.hasOwnProperty(key)) {
          ...
          if (tokens.exists) {
            ...
            var arrivalTime = moment(game.data().arrivalTime)
              .tz(sharedGame.data().timezone)
              .format("ddd MMM D LTS");
            var gameTime = moment(sharedGame.data().time)
              .tz(sharedGame.data().timezone)
              .format("ddd MMM D LTS");
            var endTime = moment(sharedGame.data().endTime)
              .tz(sharedGame.data().timezone)
              .format("ddd MMM D LTS");
            console.log(
              "Game " +
            ...
            sendPayload["notification"] = Object.assign(
              {},
              payload["notification"]
            );
            const options = Object.assign({}, payload["options"]);
            sendPayload["notification"]["icon"] =
              "https://teamsfuse.firebaseapp.com/assets/sports/" +
              team.data().sport +
              ".png";
            ...
            if (sendPayload.notification) {
              if (sendPayload.notification.title) {
                sendPayload.notification.title = sendPayload.notification.title(
                  context
                );
                sendPayload.notification.body = sendPayload.notification.body(
                  context
                );
              }
            }
            ...
            if (newTokens.length > 0) {
              ...
              allRets.push(
                admin.messaging().sendToDevice(newTokens, sendPayload, options)
              );
            }
          } else {
  ...
  for (const playerUid in input) {
    if (season.players.hasOwnProperty(playerUid)) {
      ...
      if (seasonPlayer.role === "RoleInTeam.Player") {
        ...
        if (
          seasonPlayer.jerseyNumber !== null &&
          seasonPlayer.jerseyNumber !== ""
        ) {
            ...
            if (userData.email && userData[userFlag]) {
              ...
              if (sharedGame.place.placeId !== null) {
                directionsUrl +=
                  ...
                  encodeUriComponent(sharedGame.place.placeId);
              }
              var availabilityHtml = "";
              var availabilityText = "";
              ...
              if (team.photoUrl !== null && team.photoUrl !== "") {
                ...
                teamPhotoUrl =
                  "https://www.teamsfuse.com/assets/" + team.sport + ".png";
              }
