...
exports = module.exports = functions.pubsub.topic('hourly-tick').onPublish((data, context) => {
        ...
        .then((results) => {
            ...
            for (const index in results) {
                if (Object.prototype.hasOwnProperty.call(results, index) && results[index] !== null) {
                    const doc = results[index][0];
                    const sharedGame = results[index][1];
                    ...
                    if (payload) {
                        console.log(payload);
                        promiseSeasons.push(notifyforgame.notifyForGame(doc, payload, null, false, sharedGame));
                    }
