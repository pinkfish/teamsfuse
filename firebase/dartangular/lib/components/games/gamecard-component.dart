...
class GameCardComponent implements OnInit {
  ...
  String opponent;
  Iterable<Opponent> _opponentList;
  Team _team;

  DatabaseUpdateModel _db;

  GameCardComponent(this._router, this._db);

  @override
  void ngOnInit() {
    _db.getTeamOpponents(game.teamUid).listen((event) {
      _opponentList = event;
      updateOpponent();
    });
  }
  ...
  void updateOpponent() {
    print('Checking ${_opponentList} ${game.teamUid} ');
    if (_opponentList.length > 0) {
      //opponent = _opponentList[0];
      opponent = 'unknown';
    } else {
      opponent = 'unknown';
    }
