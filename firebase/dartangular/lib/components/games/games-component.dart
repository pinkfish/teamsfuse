...
class GamesComponent implements OnInit, OnDestroy {
  ...
  @override
  void ngOnInit() {
    teams = _teamController.stream;
    _sub = _db.getTeams().listen((t) {
      _teamController.add(t);
      setupListeners(t);
    });
  ...
  void setupListeners(Iterable<Team> t) {
  ...
  void nextMonth() {
    ...
    _nextMonth = new MonthDetails(_db,
        start: curMonth.end, end: next, details: _details);
    updateLoading();
  }
  ...
  void prevMonth() {
    ...
    _prevMonth = new MonthDetails(_db,
        start: next, end: curMonth.start, details: _details);
    updateLoading();
  }
}
